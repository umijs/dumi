...
test('utoopack markdown demo rule accepts demo id query', async () => {
  registerTsResolveExtension();
  ...
  const demoRule = mdRules.find(
    (rule) => String(rule.condition?.query) === '/^\\?type=demo(?:&.*)?$/',
  );

  expect(demoRule).toBeTruthy();
  expect(demoRule.condition.query.test('?type=demo')).toBe(true);
  expect(demoRule.condition.query.test('?type=demo&demoId=demo-basic')).toBe(
    true,
  );
});
...
test('utoopack loader context includes local theme plugin files', async () => {
  const { getLoaderContextSourceFiles } = await import('./utoopackLoaders');
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'dumi-utoopack-'));
  const configPath = path.join(cwd, '.dumirc.ts');
  const themePluginPath = path.join(cwd, '.dumi/theme/plugin.ts');
  const techStackPluginPath = path.join(
    cwd,
    '.dumi/theme/plugins/tech-stack.ts',
  );

  fs.mkdirSync(path.dirname(themePluginPath), { recursive: true });
  fs.mkdirSync(path.dirname(techStackPluginPath), { recursive: true });
  fs.writeFileSync(themePluginPath, 'export default function plugin() {}');
  fs.writeFileSync(
    techStackPluginPath,
    'export default function techStackPlugin() {}',
  );

  try {
    expect(getLoaderContextSourceFiles(cwd, [configPath])).toEqual([
      configPath,
      techStackPluginPath,
      themePluginPath,
    ]);
...
test('utoopack loader context prefers exported tech stack constructors', async () => {
  registerTsResolveExtension();

  const pluginPath = require.resolve('./fixtures/exportedTechStackPlugin.ts');
  const plugin = require(pluginPath).default;
  const techStacks: any[] = [];

  plugin({
    registerTechStack(fn: () => any) {
      techStacks.push(fn());
    },
  } as any);
  ...
  expect(content).toContain('ExportedReactTechStack');
  expect(content).not.toContain('...loadTechStacksFromPlugin');

  new Function('require', 'exports', content)(require, exports);

  expect(exports.techStacks).toHaveLength(1);
  expect(exports.techStacks[0].name).toBe('exported-react');
  expect(exports.techStacks[0].generatePreviewerProps({})).toEqual({
    description: 'generated by exported tech stack',
  });
});

test('utoopack loader context restores tech stacks from exported plugins', async () => {
  registerTsResolveExtension();

  const pluginPath = require.resolve('./fixtures/unexportedTechStackPlugin.ts');
  const plugin = require(pluginPath).default;
  const techStacks: any[] = [];
  ...
  expect(exports.techStacks).toHaveLength(1);
  expect(exports.techStacks[0].name).toBe('hidden-react');
  expect(exports.techStacks[0].generatePreviewerProps({})).toEqual({
    description: 'generated by hidden tech stack',
  });
});
