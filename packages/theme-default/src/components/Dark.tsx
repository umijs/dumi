...
const Dark: FC<darkProps> = ({ darkSwitch, onDarkSwitchClick, isSideMenu }) => {
  ...
  const isSimple = theme.darkSwitch === 'simple';
  ...
  const getSimpleSvg = () => {
    if (prefersColor == 'light') {
      return (
        <button key="dumi-dark-btn-moon" title="Dark theme" onClick={ev => changeColor(ev, 'dark')} className={`__dumi-default-dark-moon`}>
          {moonSvg}
        </button>
      )
    } else {
      return (
        <button key="dumi-dark-btn-sun" title="Light theme" onClick={ev => changeColor(ev, 'light')} className={`__dumi-default-dark-sun`}>
          {sunSvg}
        </button>
      )
    }
  ...
  return (
    <div className={`__dumi-default-dark ${isSimple ? "__dumi-default-dark-simple" : ""}`}>
      {!isSimple && <div
      ...
      {!isSimple && !isSideMenu && darkSwitch && (
